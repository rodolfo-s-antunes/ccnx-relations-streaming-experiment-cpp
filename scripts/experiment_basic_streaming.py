...
class ExperimentRun:
	def __init__(self, cd):
		...
		self.streamrate = cd['blksize']*cd['streamrate']
		self.buffersize = cd['blksize']*cd['buffersize']
		return
	...
	def __generateConfs(self):
		...
		while clcount < self.clients:
			...
			auxconf += '-b %d '%self.cd['blksize']
			auxconf += '-r %d '%self.streamrate
			auxconf += '-B %d '%self.buffersize
			auxconf += '-v %s/%s-%s '%(self.ccnprefix,self.objlist[objdist.next()],STATES[clpos])
	...
	def deployConfs(self):
		...
		return
	def runWarmUp(self):
		os.system('fab -H %s experiment_basic_streaming:%s,%d,%s'%(genHosts(),self.name,self.run,self.warmfile))
		return
	def runExperiment(self):
		os.system('fab -H %s experiment_basic_streaming:%s,%d,%s'%(genHosts(),self.name,self.run,self.conffile))
		return
	def getResults(self):
	...
	def publishFiles(self):
		for fname in self.filelist:
			for country in STATES:
				auxfname = '%s-%s'%(fname,country)
				pname = '%s/%s'%(self.ccnprefix,auxfname)
				os.system('fab -H %s publish_file:%s,%s,%d,%d'%(PUBLISHER,auxfname,pname,self.bsize,self.blocks))
		return
	def resetRepo(self):
...
class Main:
   ...
   def __setParameters(self):
      ...
      self.confs.addVarParm('clients', [1, 2, 4, 8, 16, 32, 64])
      # Tamanho do bloco de memória utilizado pelos clientes
      self.confs.addFixParm('blksize', 375*1000)
      ...
      self.confs.addVarParm('filequant', [2]) # [24,30,36,42,48,54,60,66] #[42,48,54,60,66]
